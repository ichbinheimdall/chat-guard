...
async function Punish(message, GuardType, Embed) {
    ...
    if (!CanBeMuted) return console.log('UYARI - İşlem gerçekleşemedi. Sebep: "Rolleri yönet" yetkisi veya "Mute rolü" bulunamadı');
    if(!CanBeLog) return console.log('UYARI - İşlem gerçekleşemedi. Sebep: Belirli bir "Log Kanalı" bulunamadı');
    ...
    if(result === true) {
        ...
        if(GuardType === 'CharacterLimit') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, Aşırı karakter kullandığın için **'+MuteSüresi+'** Dakika susturuldun. Lütfen kurallara uymaya özen göster')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, '+message.content.length+' uzunluğunda mesaj yazdığı için **'+MuteSüresi+'** Dakika susturuldu.')).catch(() => {});
        }
        if(GuardType === 'MassPingGuard') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, Aşırı etiket attığın için **'+MuteSüresi+'** Dakika Susturuldun. Lütfen kurallara uymaya özen göster!')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, mesajında **'+message.mentions.users.size+'** kişiyi etiketlediği için **'+MuteSüresi+'** Dakika susturuldu.')).catch(() => {});
        }
        if(GuardType === 'InviteGuard') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, Sunucu daveti attığın için **'+MuteSüresi+'** Dakika Susturuldun. Lütfen kurallara uymaya özen göster')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **'+message.content+'** içerikli mesajında sunucu daveti bulunduğu için **'+MuteSüresi+'** Dakika susturuldu.')).catch(() => {});
        }
        if(GuardType === 'SpamGuard') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Spam** yaptığın için **'+MuteSüresi+'** Dakika Susturuldun. Lütfen kurallara uymaya özen göster')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Spam** yaptığı için **'+MuteSüresi+'** Dakika susturuldu.')).catch(() => {});
        }
    }
    if(result === false) {
        ...
        if(GuardType === 'CharacterLimit') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Aşırı karakter** kullanman yasak, tekrarı durumunda ceza alacaksın!')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **'+message.content.length+'** uzunluğunda mesaj yazdığı için uyarı aldı.')).catch(() => {});
        }
        if(GuardType === 'MassPingGuard') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Aşırı etiket** atman yasak, tekrarı durumunda ceza alacaksın!')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, mesajında **'+message.mentions.users.size+'** kişiyi etiketlediği için uyarı aldı.')).catch(() => {});
        }
        if(GuardType === 'InviteGuard') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Sunucu Daveti** atman yasak, tekrarı durumunda ceza alacaksın!')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **'+message.content+'** içerikli mesajında **sunucu daveti** bulunduğu için uyarı aldı.')).catch(() => {});
        }
        if(GuardType === 'SpamGuard') {
            message.channel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Spam** yapman yasak, tekrarı durumunda ceza alacaksın!')).catch(() => {});
            return LogChannel.send(Embed.setDescription('<a:red:909521226207203338> <@'+message.author.id+'>, **Spam** yaptığı için uyarı aldı.')).catch(() => {});
        }
    }
    return result;
}
